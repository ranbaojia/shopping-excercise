...
(function() {
    ...
    Swing.prototype = {
        ...
        move: function(options) {
            ...
            this.timer = setInterval(function(options) {
                this.dom.style[options.attr] = getStyle(options.attr);

            })
        },
        getStyle: function(select) {
            if (window.getComputedStyle) {
                return window.getComputedStyle(this.dom)[select];
            } else {
                return this.dom.currentStyle[select];
            }
        }
    ...
    function factory(options) {
        return new Swing(options);
    }

    window.swing = factory;
})()
...
function promiseAjax() {
 ...
 function setCookie(options) {
   ...
   if(!options.key||!options.val){
      throw new Error("设置数据不合法，没有必须参数");
   }
// 可选参数的默认值
...
function getCookie(key){
    // http规定了它的值里的分隔符是"分号"+"空格"，所以你切的时候要用"; "
       var arr=document.cookie.split('; ');
    //    循环得到的数组，由于元素为name=jia，所以还需继续切割
    console.log(arr);
